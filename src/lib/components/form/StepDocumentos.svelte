<script>
  ...
  const docs = [
    ...
    { 
      key: 'recibo_ingresos', 
      label: 'B. Recibo de Ingresos (PDF)', 
      requerido: true 
    },
    { 
      key: 'recibo_servicio_publico', 
      label: 'C. Recibo de Servicio Público (PDF)', 
      requerido: true 
    },
    { 
      key: 'recibo_inscripcion', 
      label: 'D. Comprobante de Inscripción (PDF)', 
      requerido: false 
    }
  ]

  // CORRECCIÓN: Se vuela la función flecha inline del template y se usa un despachador formal
  function handleFileChange(e, key) {
    ...
    if (!file.name.toLowerCase().endsWith('.pdf')) {
      erroresArchivos = { 
        ...erroresArchivos, 
        [key]: 'Solo se aceptan archivos PDF' 
      }
      return
    }
  ...
  // Remoción del puntero del archivo en el estado binario local
    ...
    {#each docs as doc}
      <div class="doc-item">
        ...
        {#if archivos[doc.key]}
          <div class="doc-selected">
            ...
            <button 
              type="button"
              class="doc-remove" 
              on:click={() => quitarArchivo(doc.key)}
            >
              ✕
            </button>
          </div>
        ...
        {#if erroresArchivos[doc.key]}
          <span class="req error-inline-txt">{erroresArchivos[doc.key]}</span>
        {/if}
      </div>
    {/each}
  </div>
