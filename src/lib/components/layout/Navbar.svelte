<script>
  ...
  import { logout, user, isAlumnoDual, isCoordinadorDual, isCoordinadorCarrera } from '../../stores/auth.js'
  ...
  $: esAlumnoDualStore = $isAlumnoDual
  $: esCoordinadorDualStore = $isCoordinadorDual
  $: esCoordinadorCarreraStore = $isCoordinadorCarrera

  let pathname = window.location.pathname
  $: if (sidebarOpen || $location) pathname = window.location.pathname
  ...
  function handleOverlayKeydown(e) { if (e.key === 'Escape') closeSidebar() }
  ...
  $: activeBandeja = isActive('/dual/coordinador') && !queryParams.includes('vista=empresas') && !queryParams.includes('vista=expediente')
  $: activeEmpresas = isActive('/dual/coordinador') && queryParams.includes('vista=empresas')
  $: activeExpediente = isActive('/dual/coordinador') && queryParams.includes('vista=expediente')
...
{#if sidebarOpen}
  <div class="overlay" role="button" tabindex="0" aria-label="Cerrar menú" on:click={closeSidebar} on:keydown={handleOverlayKeydown}></div>
{/if}
...
<aside class="sidebar" class:open={sidebarOpen}>
  ...
  <nav class="sidebar-nav">
    {#if esSoporteAdmin}
      ...
      <a href="/admin/usuarios" use:link class="nav-item" class:active={isActive('/admin/usuarios')} on:click={closeSidebar}>
        <span class="nav-icon">
    ...
    {#if esCoordinadorBecas || esSoporteAdmin}
      ...
      <a href="/admin/solicitudes" use:link class="nav-item" class:active={isActive('/admin/solicitudes')} on:click={closeSidebar}>
        <span class="nav-icon">
          <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 010 3.75H5.625a1.875 1.875 0 010-3.75z"/></svg>
        </span>
        Panel de becas
      </a>
      <a href="/admin/convocatoria" use:link class="nav-item" class:active={isActive('/admin/convocatoria')} on:click={closeSidebar}>
        <span class="nav-icon">
    ...
    {#if esCoordinadorDualStore || esCoordinadorCarreraStore}
      <span class="nav-section-title">Dual</span>
      {#if esCoordinadorDualStore}
        <a href="/dual/coordinador?vista=empresas" use:link class="nav-item" class:active={activeEmpresas} on:click={closeSidebar}>
          <span class="nav-icon">
        ...
        <a href="/dual/coordinador" use:link class="nav-item" class:active={activeBandeja} on:click={closeSidebar}>
          <span class="nav-icon">
        ...
        <a href="/dual/coordinador?vista=expediente" use:link class="nav-item" class:active={activeExpediente} on:click={closeSidebar}>
          <span class="nav-icon">
      ...
      {/if}
      {#if esCoordinadorCarreraStore}
        {#if !esCoordinadorDualStore}
          <a href="/dual/carrera" use:link class="nav-item" class:active={activeCarreraDirectorio} on:click={closeSidebar}>
            <span class="nav-icon">
              <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z"/></svg>
            </span>
            Expedientes y directorio
        ...
        {/if}
        <a href="/dual/carrera?vista=grupos" use:link class="nav-item" class:active={activeCarreraGrupos} on:click={closeSidebar}>
          <span class="nav-icon">
            <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zm0 9.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zm9.75-9.75A2.25 2.25 0 0115.75 3.75H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zm0 9.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z"/></svg>
          </span>
          Gestión de grupos
        </a>
      {/if}
    ...
    {#if !esCoordinadorBecas && !esSoporteAdmin && !esCoordinadorDualStore && !esCoordinadorCarreraStore}
      ...
      <a href="/foro" use:link class="nav-item" class:active={activeForo} on:click={closeSidebar}>
        <span class="nav-icon">
      ...
      <a href="/dashboard" use:link class="nav-item" class:active={activeBecaInterna} on:click={closeSidebar}>
        <span class="nav-icon">
      ...
      {#if esAlumnoDualStore}
        <a href="/dual/reportes" use:link class="nav-item" class:active={isActive('/dual/reportes')} on:click={closeSidebar}>
          <span class="nav-icon">
...
<style>
  ...
  .overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.3); z-index: 200; backdrop-filter: blur(2px); cursor: pointer; }
